...
export class DealsListPage implements OnInit {
  ...
  scrollKanban(direction: 'left' | 'right'): void {
    // Find the kanban-board element which has overflow-x: auto
    const kanbanBoard = window.document.querySelector('.kanban-board') as HTMLElement;
    
    if (kanbanBoard) {
      const scrollAmount = 300;
      if (direction === 'left') {
        kanbanBoard.scrollBy({ left: -scrollAmount, behavior: 'smooth' });
      } else {
        kanbanBoard.scrollBy({ left: scrollAmount, behavior: 'smooth' });
      }
    }
  ...
  async selectLast300AndUpdateGroup(): Promise<void> {
    ...
    const last300 = this.deals.slice(0, 277);
    const confirmAlert = await this.alertController.create({
      header: 'Select Last 300 Deals',
      message: `This will select the most recent deals (${last300.length} available) and open the group update dialog. Continue?`,
      buttons: [
        { text: 'Cancel', role: 'cancel' },
        {
          text: 'Continue',
